...
const BASE_NODES: ResearchNode[] = [
  ...
  researchNode("public-records", "Public-source record hits", "sources", "Collect and analyze public-source record hits that may corroborate identity, affiliations, location, property, business, litigation, and other context.", ["Search plan", "Juror identifiers"], ["Record hits", "Candidate facts", "Corroborating source context"], ["Stale records", "Name-only false positives"]),
  ...
  researchNode("dedupe-corroborate", "Dedupe + surface gates", "analysis", "Dedupe candidate facts, keep non-surfaced source-grounded facts separate, and only let juror-scoped or explicitly verified facts surface as juror facts.", ["Candidate facts", "Evidence snippets", "Subject scope and identity assessment"], ["Juror-surfaceable candidate facts", "Retained non-surfaced candidates", "Source-grounded fact set"], ["Premature promotion", "Over-merged people"]),
  researchNode("potential-fact-pool", "Potential Relevant Fact Pool", "analysis", "Retain sourced, non-asserted case-adjacent candidate facts that are blocked from juror-fact surfacing, preserving provenance, subject scope, identity confidence, caveats, and optional enrichment.", ["Non-surfaced candidate facts", "Source URL or evidence artifact", "Case context for enrichment"], ["potential_relevance_fact_pool", "Enrichment payload", "Source-backed snippets"], ["Noisy facts crowding the live lane", "Weak identity treated as certainty"]),
  ...
  researchNode("opposing-counsel-lens", "OC Q/A source lane", "analysis", "Keep opposing-counsel questions and answers source-labeled so they can influence strike posture without being confused with our questions or confirmed research facts.", ["Opposing-counsel Q/A entries", "Seat and juror snapshot", "Live courtroom context"], ["OC source-labeled answer signals", "OC follow-up openings", "Separate Q/A history"], ["Assuming OC priorities without courtroom proof"]),
  researchNode("signal-model", "Signal inputs / ranking helpers", "analysis", "Use deterministic helpers to turn confirmed facts, potential-relevance leads, live answers, and OC-labeled answers into question and strike-board inputs.", ["Confirmed facts", "Potential relevant facts", "Case themes", "Live answer history", "OC Q/A entries"], ["Risk signals", "Opportunity signals", "Follow-up prompts"], ["Biased weighting", "Unexplained inference"]),
  ...
  researchNode("box-live-cockpit", "Box Live Cockpit", "review", "The lawyer/operator-facing live UI: an operator surface for in-box jurors, active leads, current question, addressed seats, scratchpad, notes, Q&A, and team attention.", ["Box-ready leads", "Reviewer decisions", "Live courtroom events"], ["Live watch list", "Raised hands", "Question queue", "Scratchpad context"], ["Live state drifting from source facts"]),
  ...
  researchNode("oc-voir-dire-tracker", "OC voir dire / Q&A tracker", "review", "Capture OC questions, juror answers, rehabilitation attempts, and follow-up openings while the box is live.", ["Typed OC Q/A entries", "Live courtroom events", "Current box state"], ["OC Q&A notes", "OC follow-up openings", "Rehabilitation signals"], ["Losing sequence/context of OC questions"]),
  researchNode("live-feedback-sync", "Live feedback persistence", "review", "Persist raised-hand answers, direct notes, OC Q/A, manual strike marks, and hidden live-next envelopes back into Box state for later reload and review.", ["Current box state", "OC Q&A notes", "Strike board posture", "Manual strike marks"], ["Persisted voir_dire_events", "Updated live-next envelope", "Answer-backed Box state", "Research gaps for review"], ["Live outcomes never making it back to backend truth"]),
  ...
  researchNode("strike-signals", "Strike Signals", "output", "Separate FACT / LEAD / OURS / OC signals so strike decisions show their evidentiary and adversarial footing.", ["Risk signals", "Opportunity signals", "Lead outcomes", "OC Q&A notes", "Live answer notes"], ["FACT signals", "LEAD signals", "OURS signals", "OC signals"], ["Blending evidence with inference"]),
  ...
  researchNode("courtroom-decisions", "Manual strike/action marks", "output", "Let the operator mark peremptory or cause strike signals from the Box and use sourced research plus live answers to decide what to ask or preserve next.", ["Strike board", "Current box state", "FACT / LEAD / OURS / OC signals", "Case-level strike budgets"], ["Manual cause/peremptory strike signal", "Question / preserve posture", "Research-needed note"], ["Treating PRF or live answers as newly confirmed facts", "Losing source trace behind action posture"]),
  ...
  researchNode("coverage-eval", "Coverage + artifact eval", "output", "Score research and face-sheet artifact coverage, missing generated rows, and source support; PRF-to-Box and live-loop gaps remain review signals rather than a fully automated eval contract.", ["Face sheet", "Source graph", "Research snapshot", "Live feedback notes"], ["Coverage report", "Missing artifact rows", "Regression signals", "Review gaps"], ["Green checks on stale artifacts"]),
];
...
const BASE_EDGES: ResearchEdge[] = [
  ...
  researchEdge("opposing-counsel-lens", "signal-model", "OC source context"),
  ...
  researchEdge("opposing-counsel-lens", "oc-voir-dire-tracker", "capture lane"),
  ...
  researchEdge("opposing-counsel-lens", "strike-signals", "OC source labels"),
